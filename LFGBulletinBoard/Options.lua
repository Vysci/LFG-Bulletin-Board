...
function GBB.OptionsInit ()
	...
	GBB.OptionsBuilder.AddSpacerToPanel()
	--override the default `SetSavedValue` behavior to clamp the TimeOut value above 60 (previously done in `GBB.OptionsUpdate()`)
	local timeOutSetting = CreateEditBoxNumber("TimeOut",150,50,60)
	local savedVarHandle = GBB.OptionsBuilder.GetSavedVarHandle(GBB.DB, "TimeOut")
	function timeOutSetting:SetSavedValue(value) 
		savedVarHandle:SetValue(Clamp(value, 60, math.huge))
	end
	GBB.OptionsBuilder.AddSpacerToPanel()
